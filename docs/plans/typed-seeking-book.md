# План реализации страницы ГЭС

## Обзор

Создание новой страницы детализации ГЭС (дашборд) с основными показателями и возможностью подробного просмотра данных через табы.
...
1. **Дашборд-секция** (верхняя часть):
    - Основная информация о ГЭС (название, родительская организация)
    - Карточки с KPI: количество аварий, количество водосбросов, текущая мощность
    - Список агрегатов с телеметрией (мощность, статус)
    - Место для картинки ГЭС (заглушка на будущее)
    ...
    └── situation-center/
        └── ges/
            └── ges-detail/                 # Новый модуль
                ...
                │   ├── ges-aggregates/     # Список агрегатов
...
- Загружает основную информацию о ГЭС по ID из route params
- Содержит дашборд-секцию и табы
- Управляет переключением между ГЭС через dropdown

### 2. GesDashboardComponent

- Отображает KPI карточки
- Список агрегатов с телеметрией
- Мнемосхему станции
...
- Интерактивные элементы (клик по агрегату показывает детали)
