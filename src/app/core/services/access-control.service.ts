...
export class AccessControlService extends ApiService {
    ...
    getCards(params?: { employee_id?: number; status?: string; search?: string }): Observable<AccessCard[]> {
        ...
        return this.http.get<AccessCard[]>(this.BASE_URL + ACCESS_CONTROL + '/cards', { params: httpParams });
    }

    getCard(id: number): Observable<AccessCard> {
        return this.http.get<AccessCard>(this.BASE_URL + ACCESS_CONTROL + '/cards/' + id);
    }

    createCard(data: Partial<AccessCard>): Observable<AccessCard> {
        return this.http.post<AccessCard>(this.BASE_URL + ACCESS_CONTROL + '/cards', data);
    }

    updateCard(id: number, data: Partial<AccessCard>): Observable<AccessCard> {
        return this.http.patch<AccessCard>(this.BASE_URL + ACCESS_CONTROL + '/cards/' + id, data);
    }

    blockCard(id: number, reason: string): Observable<AccessCard> {
        return this.http.post<AccessCard>(this.BASE_URL + ACCESS_CONTROL + '/cards/' + id + '/block', { reason });
    }

    unblockCard(id: number): Observable<AccessCard> {
        return this.http.post<AccessCard>(this.BASE_URL + ACCESS_CONTROL + '/cards/' + id + '/unblock', {});
    }

    // Access Zones
    getZones(): Observable<AccessZone[]> {
        return this.http.get<AccessZone[]>(this.BASE_URL + ACCESS_CONTROL + '/zones');
    }

    createZone(data: Partial<AccessZone>): Observable<AccessZone> {
        return this.http.post<AccessZone>(this.BASE_URL + ACCESS_CONTROL + '/zones', data);
    }

    updateZone(id: number, data: Partial<AccessZone>): Observable<AccessZone> {
        return this.http.patch<AccessZone>(this.BASE_URL + ACCESS_CONTROL + '/zones/' + id, data);
    }
    ...
    getLogs(params?: { employee_id?: number; zone_id?: number; direction?: string; status?: string; date_from?: string; date_to?: string }): Observable<AccessLog[]> {
        ...
        return this.http.get<AccessLog[]>(this.BASE_URL + ACCESS_CONTROL + '/logs', { params: httpParams });
    }

    // Access Requests
    getRequests(): Observable<AccessRequest[]> {
        return this.http.get<AccessRequest[]>(this.BASE_URL + ACCESS_CONTROL + '/requests');
    }

    createRequest(data: Partial<AccessRequest>): Observable<AccessRequest> {
        return this.http.post<AccessRequest>(this.BASE_URL + ACCESS_CONTROL + '/requests', data);
    }

    approveRequest(id: number): Observable<AccessRequest> {
        return this.http.post<AccessRequest>(this.BASE_URL + ACCESS_CONTROL + '/requests/' + id + '/approve', {});
    }

    rejectRequest(id: number, reason: string): Observable<AccessRequest> {
        return this.http.post<AccessRequest>(this.BASE_URL + ACCESS_CONTROL + '/requests/' + id + '/reject', { reason });
    }
}
