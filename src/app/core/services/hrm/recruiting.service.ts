...
export class RecruitingService {
    ...
    private get API_URL(): string {
        return this.configService.apiBaseUrl + '/hrm/recruiting';
    }
    ...
    getVacancies(params?: { status?: string; department_id?: number }): Observable<Vacancy[]> {
        ...
        return this.http.get<Vacancy[]>(`${this.API_URL}/vacancies`, { params: httpParams });
    }

    getVacancyById(id: number): Observable<Vacancy> {
        return this.http.get<Vacancy>(`${this.API_URL}/vacancies/${id}`);
    }

    createVacancy(payload: VacancyPayload): Observable<Vacancy> {
        return this.http.post<Vacancy>(`${this.API_URL}/vacancies`, payload);
    }

    updateVacancy(id: number, payload: Partial<VacancyPayload>): Observable<Vacancy> {
        return this.http.patch<Vacancy>(`${this.API_URL}/vacancies/${id}`, payload);
    }

    deleteVacancy(id: number): Observable<void> {
        return this.http.delete<void>(`${this.API_URL}/vacancies/${id}`);
    }

    publishVacancy(id: number): Observable<Vacancy> {
        return this.http.post<Vacancy>(`${this.API_URL}/vacancies/${id}/publish`, {});
    }

    closeVacancy(id: number): Observable<Vacancy> {
        return this.http.post<Vacancy>(`${this.API_URL}/vacancies/${id}/close`, {});
    }
    ...
    getCandidates(params?: { vacancy_id?: number; status?: string }): Observable<Candidate[]> {
        ...
        if (params?.status) httpParams = httpParams.set('status', params.status);
        return this.http.get<Candidate[]>(`${this.API_URL}/candidates`, { params: httpParams });
    }
    ...
    updateCandidate(id: number, payload: Partial<CandidatePayload>): Observable<Candidate> {
        return this.http.patch<Candidate>(`${this.API_URL}/candidates/${id}`, payload);
    }
    ...
    updateCandidateStatus(id: number, status: string, notes?: string): Observable<Candidate> {
        return this.http.patch<Candidate>(`${this.API_URL}/candidates/${id}/status`, { status, notes });
    }

    getCandidateInterviews(candidateId: number): Observable<Interview[]> {
        return this.http.get<Interview[]>(`${this.API_URL}/candidates/${candidateId}/interviews`);
    }

    // Interviews
    getInterviews(): Observable<Interview[]> {
        return this.http.get<Interview[]>(`${this.API_URL}/interviews`);
    }

    createInterview(interview: Partial<Interview>): Observable<Interview> {
        return this.http.post<Interview>(`${this.API_URL}/interviews`, interview);
    }

    updateInterview(id: number, data: Partial<Interview>): Observable<Interview> {
        return this.http.patch<Interview>(`${this.API_URL}/interviews/${id}`, data);
    }
