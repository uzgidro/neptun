...
export class VacationService {
    ...
    private get API_URL(): string {
        return this.configService.apiBaseUrl + '/hrm/vacations';
    }
    ...
    getAll(params?: { status?: string; year?: number }): Observable<Vacation[]> {
        ...
        return this.http.get<Vacation[]>(this.API_URL, { params: httpParams });
    }

    getById(id: number): Observable<Vacation> {
        return this.http.get<Vacation>(`${this.API_URL}/${id}`);
    }

    create(payload: VacationPayload): Observable<Vacation> {
        return this.http.post<Vacation>(this.API_URL, payload);
    }

    update(id: number, payload: Partial<VacationPayload>): Observable<Vacation> {
        return this.http.patch<Vacation>(`${this.API_URL}/${id}`, payload);
    }
    ...
    approve(id: number): Observable<Vacation> {
        return this.http.post<Vacation>(`${this.API_URL}/${id}/approve`, {});
    }

    reject(id: number, reason: string): Observable<Vacation> {
        return this.http.post<Vacation>(`${this.API_URL}/${id}/reject`, { reason });
    }

    cancel(id: number): Observable<Vacation> {
        return this.http.post<Vacation>(`${this.API_URL}/${id}/cancel`, {});
    }

    getBalance(employeeId: number, year?: number): Observable<VacationBalance> {
        let params = new HttpParams();
        if (year) params = params.set('year', year.toString());
        return this.http.get<VacationBalance>(`${this.API_URL}/balance/${employeeId}`, { params });
    }

    getAllBalances(year?: number): Observable<VacationBalance[]> {
        let params = new HttpParams();
        if (year) params = params.set('year', year.toString());
        return this.http.get<VacationBalance[]>(`${this.API_URL}/balances`, { params });
    }

    getPendingApprovals(): Observable<Vacation[]> {
        return this.http.get<Vacation[]>(`${this.API_URL}/pending`);
    }
    ...
    getCalendar(month: number, year: number): Observable<Vacation[]> {
        ...
        return this.http.get<Vacation[]>(`${this.API_URL}/calendar`, { params });
    }
}
