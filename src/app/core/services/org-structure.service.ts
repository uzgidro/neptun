...
const MOCK_UNITS: OrgUnit[] = [
    {
        id: 1, name: 'АО «МолокоПром»', code: 'UGE', type: 'company', parent_id: null,
    ...
    {
        id: 7, name: 'Сырдарьинский молокозавод', code: 'SYRDARYA', type: 'branch', parent_id: 1,
        employee_count: 520, is_active: true, created_at: '2000-01-01', location: 'Сырдарья'
    },
    {
        id: 8, name: 'Зарафшанский молокозавод', code: 'ZARAFSHAN', type: 'branch', parent_id: 1,
...
export class OrgStructureService extends ApiService {
    ...
    getOrgUnit(id: number): Observable<OrgUnit> {
        if (USE_MOCK) {
            return of(MOCK_UNITS.find(u => u.id === id) || MOCK_UNITS[0]).pipe(delay(150));
        }
        return this.http.get<OrgUnit>(BASE_URL + ORG_STRUCTURE + '/units/' + id);
    }

    createOrgUnit(data: Partial<OrgUnit>): Observable<OrgUnit> {
        if (USE_MOCK) return of({ ...MOCK_UNITS[1], ...data, id: Date.now() } as OrgUnit).pipe(delay(200));
    ...
    updateOrgUnit(id: number, data: Partial<OrgUnit>): Observable<OrgUnit> {
        if (USE_MOCK) return of({ ...MOCK_UNITS[1], ...data, id } as OrgUnit).pipe(delay(200));
