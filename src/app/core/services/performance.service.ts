...
export class PerformanceService extends ApiService {
    ...
    getGoals(): Observable<PerformanceGoal[]> {
        return this.http.get<PerformanceGoal[]>(this.BASE_URL + GOALS);
    }

    getGoal(id: number): Observable<PerformanceGoal> {
        return this.http.get<PerformanceGoal>(this.BASE_URL + GOALS + '/' + id);
    }

    createGoal(payload: GoalPayload): Observable<PerformanceGoal> {
        return this.http.post<PerformanceGoal>(this.BASE_URL + GOALS, payload);
    }

    updateGoal(id: number, payload: GoalPayload): Observable<PerformanceGoal> {
        return this.http.patch<PerformanceGoal>(this.BASE_URL + GOALS + '/' + id, payload);
    }

    deleteGoal(id: number): Observable<any> {
        return this.http.delete(this.BASE_URL + GOALS + '/' + id);
    }

    // Reviews CRUD
    getReviews(): Observable<PerformanceReview[]> {
        return this.http.get<PerformanceReview[]>(this.BASE_URL + REVIEWS);
    }

    getReview(id: number): Observable<PerformanceReview> {
        return this.http.get<PerformanceReview>(this.BASE_URL + REVIEWS + '/' + id);
    }

    createReview(payload: PerformanceReviewPayload): Observable<PerformanceReview> {
        return this.http.post<PerformanceReview>(this.BASE_URL + REVIEWS, payload);
    }

    updateReview(id: number, payload: Partial<PerformanceReviewPayload>): Observable<PerformanceReview> {
        return this.http.patch<PerformanceReview>(this.BASE_URL + REVIEWS + '/' + id, payload);
    }

    deleteReview(id: number): Observable<any> {
        return this.http.delete(this.BASE_URL + REVIEWS + '/' + id);
    }
}
