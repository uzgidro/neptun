...
export class SalaryService extends ApiService {
    ...
    getSalaryStructures(): Observable<EmployeeSalaryStructure[]> {
        return this.http.get<EmployeeSalaryStructure[]>(this.BASE_URL + SALARY + '/structures');
    }

    getSalaryStructure(employeeId: number): Observable<EmployeeSalaryStructure> {
        return this.http.get<EmployeeSalaryStructure>(this.BASE_URL + SALARY + '/structures/' + employeeId);
    }

    updateSalaryStructure(employeeId: number, data: Partial<EmployeeSalaryStructure>): Observable<EmployeeSalaryStructure> {
        return this.http.patch<EmployeeSalaryStructure>(this.BASE_URL + SALARY + '/structures/' + employeeId, data);
    }

    // Bonuses
    getBonuses(): Observable<EmployeeBonus[]> {
        return this.http.get<EmployeeBonus[]>(this.BASE_URL + SALARY + '/bonuses');
    }

    createBonus(data: Partial<EmployeeBonus>): Observable<EmployeeBonus> {
        return this.http.post<EmployeeBonus>(this.BASE_URL + SALARY + '/bonuses', data);
    }

    deleteBonus(id: number): Observable<any> {
        return this.http.delete(this.BASE_URL + SALARY + '/bonuses/' + id);
    }

    // Deductions
    getDeductions(): Observable<EmployeeDeduction[]> {
        return this.http.get<EmployeeDeduction[]>(this.BASE_URL + SALARY + '/deductions');
    }

    createDeduction(data: Partial<EmployeeDeduction>): Observable<EmployeeDeduction> {
        return this.http.post<EmployeeDeduction>(this.BASE_URL + SALARY + '/deductions', data);
    }

    deleteDeduction(id: number): Observable<any> {
        return this.http.delete(this.BASE_URL + SALARY + '/deductions/' + id);
    }

    // Calculations
    calculateSalary(month: number, year: number): Observable<BatchCalculationResult> {
        return this.http.post<BatchCalculationResult>(this.BASE_URL + SALARY + '/calculate', { month, year });
    }

    approveSalary(month: number, year: number): Observable<any> {
        return this.http.post(this.BASE_URL + SALARY + '/approve', { month, year });
    }

    // Export
    exportPayslips(month: number, year: number): Observable<Blob> {
        return this.http.get(this.BASE_URL + SALARY + '/export', {
