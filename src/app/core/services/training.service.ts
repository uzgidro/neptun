...
export class TrainingService extends ApiService implements CrudService<Training, TrainingPayload> {
    getAll(): Observable<Training[]> {
        return this.http.get<Training[]>(this.BASE_URL + TRAININGS);
    }

    getById(id: number): Observable<Training> {
        return this.http.get<Training>(this.BASE_URL + TRAININGS + '/' + id);
    }

    create(payload: TrainingPayload): Observable<Training> {
        return this.http.post<Training>(this.BASE_URL + TRAININGS, payload);
    }

    update(id: number, payload: TrainingPayload): Observable<Training> {
        return this.http.patch<Training>(this.BASE_URL + TRAININGS + '/' + id, payload);
    }

    delete(id: number): Observable<void> {
        return this.http.delete<void>(this.BASE_URL + TRAININGS + '/' + id);
    }
