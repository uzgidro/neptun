...
@Component({
    ...
    template: `
        <div class="relative" *ngIf="userName">
            ...
            <div class="hidden absolute top-13 right-0 w-72 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-border origin-top shadow-md">
                ...
                <hr class="my-4 border-t border-surface" />
                <a (click)="logout()" class="cursor-pointer flex items-center gap-2 p-2 rounded-border-sm hover:bg-surface-100 dark:hover:bg-surface-800/70"><i class="pi pi-sign-out"></i><span>Logout</span></a>
            </div>
        </div>
    `
})
